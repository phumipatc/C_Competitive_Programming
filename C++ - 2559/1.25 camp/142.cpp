...
int main()
{
    ...
    while(q--){
        ...
        temp.i=si,temp.j=sj,temp.d=0,ch=1;
        b.push(temp);
        while(!b.empty()){
            i=b.front().i; j=b.front().j; d=b.front().d;   //dequeue
            b.pop();
            if(i==ei && j==ej){
                printf("%d\n",d);  ch=0;  break;
            }
            if(i-1>=0 && (a[i-1][j]=='.'||a[i-1][j]=='B') && c[i-1][j]!=1){
                c[i-1][j]=1;    temp.i=i-1,temp.j=j,temp.d=d+1; b.push(temp);
            }
            if(j-1>=0 && (a[i][j-1]=='.'||a[i][j-1]=='B') && c[i][j-1]!=1){
                c[i][j-1]=1;    temp.i=i,temp.j=j-1,temp.d=d+1; b.push(temp);
            }
